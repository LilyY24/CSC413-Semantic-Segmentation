...
def iou(preds, labels, C, ignore=None):
    """
    ...
    for i in range(C):
        for pred, label in zip(preds, labels):
            iou = []
            if i != ignore:
                intersection = sum(((label == i) & (pred == i)))
                union = sum(((label == i) | ((pred == i) & (label != ignore))))
                if union == 0:
                    iou.append(0.0)
                else:
                    iou.append(float(intersection) / float(union))
        ious.append(iou)
    ious = [sum(iou)/len(iou) for iou in ious]
...
def main():
    ...
    file_list = os.path.join(root, "ImageSets/Segmentation", "test_vehicle" + ".txt")
    file_list = [line.rstrip().split(' ')[0] for line in
                 tuple(open(file_list, "r"))]
    path = 'data/VOCtrainval_11-May-2012/VOCdevkit/VOC2012/JPEGImages'
    target_path = 'data/VOCtrainval_11-May-2012/VOCdevkit/VOC2012/SegmentationClass'

    image_files = sorted([path + "/" + f + ".jpg" for f in file_list])
    target_files = sorted([target_path + "/" + f + ".png" for f in file_list])
    #image_files = sorted(glob(os.path.join(args.images, f'*.{args.extension}')))
    with torch.no_grad():
        tbar = tqdm(zip(image_files, target_files))
        target_list = []
        prediction_list = []
        pix_acc = []
        for img_file, tar_file in tbar:
            image = Image.open(img_file).convert('RGB')
            input = normalize(to_tensor(image)).unsqueeze(0)
            target = to_tensor(np.asarray(Image.open(tar_file), dtype=np.float32)).type(torch.LongTensor)
            target_list.append(target.numpy().flatten())
            ...
            prediction_list.append(prediction.flatten())
            pix_acc.append(pixel_accuracy(prediction.flatten(), target.numpy().flatten()))
            save_images(image, prediction, args.output, img_file, palette)
        class_iou = iou(prediction_list, target_list, num_classes)
        miou = sum(class_iou)/len(class_iou)
        pixacc = sum(pix_acc)/len(pix_acc)

        print("mIoU is {}".format(miou))
        print("pixel accuracy is {}".format(pixacc))

        print('class IoU is {}'.format(class_iou))


def parse_arguments():
    parser = argparse.ArgumentParser(description='Inference')
    parser.add_argument('-c', '--config', default='VOC_vehicle.json',type=str,
                        help='The config used to train the model')
    parser.add_argument('-mo', '--mode', default='multiscale', type=str,
                        help='Mode used for prediction: either [multiscale, sliding]')
    parser.add_argument('-m', '--model', default='model_vehicle.pth', type=str,
                        help='Path to the .pth model checkpoint to be used in the prediction')
    parser.add_argument('-i', '--images', default='data/VOCtrainval_11-May-2012/VOCdevkit/VOC2012/JPEGImages', type=str,
